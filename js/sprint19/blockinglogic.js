...
// For adding the array for new categories. This won't be needed in our new approach
// cliConfig.info.categories.forEach(function (category) {
...
CKY_BLACKLIST = [];
CKY_WHITELIST = [];
const ckyconsent = getCategoryCookie("cky-consent") ? getCategoryCookie("cky-consent") : "no";
...
const patterns = { blacklist: window.CKY_BLACKLIST, whitelist: window.CKY_WHITELIST };
...
function isOnWhitelist(src) {
  return patterns.whitelist.some(function (pattern) {
    return pattern.test(src);
  });
};
...
var cookieYes = {
  ...
  unblock: function () {
      ...
      _toConsumableArray(backupRemovedScripts.blacklisted).forEach(function (script, index) {
          if (script.src && isOnWhitelist(script.src)) {
