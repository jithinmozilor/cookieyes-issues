...
function bannerActiveCheck() {
  ...
  if (!isActiveCheckCookiePresent && window.fetch && window.Promise) {
      fetch("https://active.cookieyes.com/api/a260231864e4c56338cac906/log", { method: "POST" }).catch(function (err) {
          console.error(err);
      });
...
var cliConfig = {
  options: {
      ...
      display: { gdpr: { notice: true, title: false, buttons: { accept: true, reject: true, settings: true, readMore: false }, noticeToggler: false } },
